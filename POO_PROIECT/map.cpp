...
void ProceduralMapGenerator::loadMap(Map &map) {
    for (int i = 0; i < map.rows; i++) {
        for (int j =0; j< map.columns; j++) {
            ...
            switch(nr) {
                case 1: map.grid[i][j] = SPACE;break;
                case 2: map.grid[i][j] = WALL;break;
                case 3: map.grid[i][j] = HUB; break;
                case 4: map.grid[i][j] = STATION; break;
                case 5: map.grid[i][j] = DESTINATION; break;
            }
        }
    }
}
